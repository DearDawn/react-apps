...
export class Bubble {
  ...
  updateBubbleBg () {
    // 更新对话气泡的宽度和高度
    this.bubbleBg.width = Math.min(Math.max(this.text.width + 50, this.bubbleBg.width), 200);
    this.bubbleBg.height = Math.min(Math.max(this.text.height * 2 + 50, this.bubbleBg.height), 200);
  }
  ...
  setText (text = '') {
    ...
    this.updateBubbleBg();
    this.timer = setTimeout(this.destroyCb, 3000);
  }
